...
When the mouse is inside the graph, and the Rectangle Selection tool in the toolbar is consequently activated, it's possible to draw **Regions** in the dot plot to select the contained events. Then press the "Set Region" Button (1) to store its coordinate. The region will appear in red in the dot plot, will be added to the ImageJ ROI Manager and stored in an internal **Region List**.
